...
Current  : %s (%.2f%%)
%s
""" % (jobid if jobid else '', ' '.join(sys.argv), self.started, elapsed_time, est_remaining, self.total, current,
       pct * 100, '\n%s' % self.extra if self.extra else '')


class _ETA(object):
    def __init__(self, total, modulo=None, fileobj=None, window=50, step=1, prog_bar_length=20,
                 min_ms_between_updates=None):
